...
class ItemGridPanel extends ConsumerWidget {
  ...
  const ItemGridPanel({super.key, required this.searchFocusNode});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Column(
      children: [
        ...
        // ---------------- Items Grid ----------------
        const Expanded(child: _ItemGrid()),
      ],
    );
  }
}
  ...
  void _showStockWarning(String message) {
    ...
    ScaffoldMessenger.of(context)
      ...
      ..showSnackBar(
        ...
        if (mounted) _stockWarningShown = false;
      });
  }

  Future<void> _handleAddItem(Item item) async {
    final addItemToCart = ref.read(addItemToCartProvider);
    final cartNotifier = ref.read(checkoutProvider.notifier);

    // Get current quantity of this item in cart
    final cartState = ref.read(checkoutProvider);
    final currentQty = cartState.activeCheckout.items
